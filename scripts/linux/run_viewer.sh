...
BIN="${SCRIPT_DIR}/cog_soc_env_space_viewer"

if [[ ! -x "${BIN}" ]]; then
  echo "Error: viewer executable not found at: ${BIN}" >&2
  exit 1
fi
...
DEFAULT_JSON="${SCRIPT_DIR}/../output/simulation.json"
if [[ $# -eq 0 && -f "${DEFAULT_JSON}" ]]; then
  exec "${BIN}" "${DEFAULT_JSON}"
fi
